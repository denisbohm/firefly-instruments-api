...
def GetDeviceIntProperty(dev_ref, key):
    ...
    if cf.CFGetTypeID(type_ref) != cf.CFNumberGetTypeID():
        raise IOError('Expected number type, got {}'.format(
...
def GetDeviceStringProperty(dev_ref, key):
    ...
    if cf.CFGetTypeID(type_ref) != cf.CFStringGetTypeID():
        raise IOError('Expected string type, got {}'.format(
            cf.CFGetTypeID(type_ref)))
...
class MacOsHidDevice(HidDevice):
    ...
    def Enumerate():
        ...
        if not hid_mgr:
            raise IOError('Unable to obtain HID manager reference')
        ...
        if not device_set_ref:
            raise IOError('Failed to obtain devices from HID manager')
    ...
    @staticmethod
    def open(vid, pid):
        descriptors = MacOsHidDevice.Enumerate()
        for descriptor in descriptors:
            if descriptor.vendor_id == vid and descriptor.product_id == pid:
                return MacOsHidDevice(descriptor.path)
    ...
    def __init__(self, path):
        ...
        if not device_entry:
            raise IOError('Device path does not match any HID device on '
                                    'the system')

        self.device_handle = iokit.IOHIDDeviceCreate(K_CF_ALLOCATOR_DEFAULT,
                                                     device_entry)
        if not self.device_handle:
            raise IOError('Failed to obtain device handle from registry '
                                    'entry')
        ...
        if result != K_IO_RETURN_SUCCESS:
            raise IOError('Failed to open device for communication: {}'
                                    .format(result))
        ...
        if not self.internal_max_in_report_len:
            raise IOError('Unable to obtain max in report size')
        ...
        if not self.internal_max_out_report_len:
            raise IOError('Unable to obtain max out report size')
...
#        packet_len = self.internal_max_out_report_len
        ...
        if result != K_IO_RETURN_SUCCESS:
            raise IOError('Failed to write report to device')
